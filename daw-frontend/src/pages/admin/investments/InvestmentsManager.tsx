...
export default function InvestmentsManager() {
  ...
  const sortedCompanies = useMemo(() => {
    ...
    return [...localCompanies].sort((a, b) => {
      ...
      // Prioritas 1: Selalu taruh kotak "Add New" di paling atas
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-in fade-in duration-500 pb-12">
      ...
      <div className="bg-white rounded-b-xl border border-t-0 border-slate-200 shadow-sm p-6 lg:p-8 min-h-[500px] transition-all duration-500">
        ...
        {/* TAB 2: NETWORK ECOSYSTEM (NESTED CATEGORIES) */}
        {activeTab === "companies" && (
              ...
              <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-5 space-y-3 animate-in slide-in-from-top-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <input type="text" placeholder="Deskripsi singkat" value={newCategoryData.description}
                    ...
                    className="px-3 py-2 text-sm rounded-lg border border-emerald-300 bg-white focus:ring-2 focus:ring-emerald-400/30 outline-none" />
                  <select value={newCategoryData.icon} onChange={(e) => setNewCategoryData(p => ({ ...p, icon: e.target.value }))}
                    className="px-3 py-2 text-sm rounded-lg border border-emerald-300 bg-white outline-none">
                    <option value="Briefcase">💼 Briefcase</option>
                    <option value="Coffee">☕ Coffee / F&B</option>
                    <option value="Globe2">🌐 Globe / Finance</option>
                        ...
                        {isEditingThisCat ? (
                          <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                            <input type="text" value={cat.name} autoFocus
                              ...
                              className="px-2 py-1 text-sm font-bold border border-daw-green rounded-md bg-white outline-none w-40" />
                            <input type="text" value={cat.description} placeholder="Deskripsi..."
                              onChange={e => setLocalCategories(prev => prev.map(c => c.id === cat.id ? { ...c, description: e.target.value } : c))}
                              className="px-2 py-1 text-xs border border-slate-300 rounded-md bg-white outline-none w-56" />
                            <select value={cat.icon}
                              onChange={e => setLocalCategories(prev => prev.map(c => c.id === cat.id ? { ...c, icon: e.target.value } : c))}
                              className="px-2 py-1 text-xs border border-slate-300 rounded-md bg-white outline-none">
                              <option value="Briefcase">💼</option><option value="Coffee">☕</option>
                              <option value="Globe2">🌐</option><option value="GraduationCap">🎓</option>
                              <option value="Building">🏢</option>
                            </select>
                            <button className="px-3 py-1 bg-daw-green text-white text-[10px] font-bold rounded-md"
                              onClick={async () => {
                            ...
                            <button className="px-2 py-1 text-slate-500 text-[10px] font-bold" onClick={() => setEditingCategoryId(null)}>Batal</button>
                          </div>
                        ) : (
                          <>
