...
interface CompaniesTabProps {
  ...
  setNewCategoryData: React.Dispatch<
    React.SetStateAction<{ name: string; description: string; icon: string }>
  >;
  editingCategoryId: number | string | null;
  setEditingCategoryId: React.Dispatch<
...
export default function CompaniesTab({
  ...
  const [currentCropFile, setCurrentCropFile] = React.useState<{
    file: File;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between mb-4 border-b border-slate-100 pb-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Kelola kategori investasi dan perusahaan afiliasi secara hierarkis.
          </p>
        </div>
        {isEditing && (
          <button
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-5 space-y-3 animate-in slide-in-from-top-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <input
              ...
              onChange={(e) =>
                setNewCategoryData((p) => ({ ...p, name: e.target.value }))
              }
          ...
          <div className="bg-white/60 p-3 rounded-lg border border-emerald-200/60">
            <label className="text-[10px] font-black text-emerald-800/70 uppercase tracking-widest mb-2 block">
              Pilih Ikon Kategori
            </label>
            <div className="flex flex-wrap gap-2 max-h-48 overflow-y-auto scrollbar-thin pr-2">
              {CATEGORY_ICONS.map((iconName) => {
                const IconComponent =
                  (LucideIcons as any)[iconName] || LucideIcons.Briefcase;
                return (
                  <button
                    key={iconName}
                    type="button"
                    title={iconName}
                    onClick={() =>
                      setNewCategoryData((p) => ({ ...p, icon: iconName }))
                    }
          ...
          <div className="flex gap-2">
            <button
              disabled={!newCategoryData.name.trim() || isSaving}
              onClick={async () => {
      ...
      {localCategories.map((cat) => {
        const companiesInCat = sortedCompanies.filter(
        ...
        return (
          <div
            ...
            {/* CATEGORY HEADER */}
            <div
              className="flex items-center justify-between px-5 py-4 bg-slate-50 border-b border-slate-200 cursor-pointer select-none"
              onClick={() =>
                setLocalCategories((prev) =>
                  prev.map((c) =>
                    c.id === cat.id ? { ...c, isCollapsed: !c.isCollapsed } : c,
                  ),
                )
              }>
              <div className="flex items-center gap-3">
                <ChevronDown
                  className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${cat.isCollapsed ? "-rotate-90" : ""}`}
                />
                {(() => {
                  const CatIcon =
                    (LucideIcons as any)[cat.icon] || LucideIcons.Briefcase;
                    ...
                    <div
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <div className="flex gap-2 ml-auto">
                          <button
                            className="px-4 py-1.5 bg-daw-green hover:bg-emerald-600 text-white text-xs font-bold rounded-md shadow-sm transition-colors"
                            onClick={async () => {
                              const tid = toast.loading("Memperbarui...");
                              try {
                                await api.put(
                                  `/investments/categories/${cat.id}`,
                                  {
                                    name: cat.name,
                          ...
                          <button
                            className="px-4 py-1.5 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 text-xs font-bold rounded-md transition-colors"
                ...
                {isEditing && !isEditingThisCat && (
                  <>
                    <button
                      ...
                      className="p-1 text-slate-400 hover:text-daw-green rounded transition-colors"
                      title="Edit Kategori">
                      <Pencil className="w-3.5 h-3.5" />
                    </button>
                    <button
                      onClick={() => {
                        if (companiesInCat.length > 0)
                          return toast.error(
                            "Tidak bisa hapus kategori yang masih memiliki perusahaan.",
                          );
                          ...
                          action: {
                            ...
                            onClick: async () => {
                              ...
                              try {
                                ...
                                toast.success("Kategori dihapus!", { id: tid });
                                await refreshData();
                              } catch (err: any) {
                                toast.error(
                                  err.response?.data?.message || "Gagal",
            ...
            {!cat.isCollapsed && (
              <div className="p-5 space-y-4">
                {isEditing && (
                  <button
                    onClick={() => addCompany(cat.id as number)}
                    disabled={isSaving}
                    ...
                    <Plus className="w-3.5 h-3.5" /> Tambah Perusahaan ke{" "}
                    {cat.name}
                  </button>
                )}
                      ...
                      <div
                        ...
                        {isNeedsRevision && draft && (
                          <div className="w-full bg-red-50 border-l-4 border-l-red-500 border-y border-r border-red-200 rounded-lg p-3 shadow-sm animate-in fade-in">
                            ...
                            <p className="text-[10px] text-red-800 bg-white/60 p-2 rounded border border-red-200/50 mb-2">
                              "
                        ...
                        <div className="flex gap-3 items-start w-full">
                          ...
                          <fieldset
                            ...
                            <MagicTranslationField
                              ...
                              onChange={(v) =>
                                updateCompany(company.id, "terjemahanDesc", v)
                              }
        ...
        <div className="py-16 text-center text-slate-500 flex flex-col items-center gap-3 bg-slate-50 rounded-xl border border-slate-200 border-dashed">
          ...
          <div>
            ...
            <p className="text-xs mt-1">
              Klik "Tambah Kategori" untuk memulai membangun ekosistem
              investasi.
            </p>
          </div>
        </div>
