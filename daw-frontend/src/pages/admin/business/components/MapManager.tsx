            ...
            <div className="space-y-3 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar">
              {formData.mapMarkers.map((marker: MapMarker, index: number) => (
                <div
                  ...
                  className={`border border-slate-200 rounded-xl relative overflow-hidden bg-white shadow-sm transition-all duration-300 ${
                    !isLocked ? "hover:border-slate-300 hover:shadow-md" : ""
                  } ${itemLockStyles}`}
                  style={{
                    borderLeft: `4px solid ${categoryMap[marker.categoryId] || "#cbd5e1"}`
                  }}
                >
                  {/* Card Header */}
                  <div className="flex justify-between items-center bg-slate-50/70 px-4 py-2 border-b border-slate-100">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-black text-slate-500 uppercase tracking-wider">
                        Lokasi #{index + 1}
                      </span>
                      <span className="text-[8px] font-mono bg-slate-200/60 text-slate-500 px-1.5 py-0.5 rounded">
                        X: {marker.dotX} | Y: {marker.dotY}
                      </span>
                    </div>
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() => handleCopyCoords(`${marker.dotX}, ${marker.dotY}`)}
                        className="p-1 hover:bg-slate-200/50 text-slate-400 hover:text-slate-600 rounded transition-colors"
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        <input
                          className="w-full text-xs font-semibold bg-white border border-slate-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-daw-green/10 focus:border-daw-green/40 transition-all disabled:opacity-75 disabled:bg-slate-50"
                          value={marker.title || ""}
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="relative">
                          <select
                            className="w-full text-xs bg-white border border-slate-200 rounded-lg pl-3 pr-8 py-2 outline-none appearance-none cursor-pointer focus:ring-2 focus:ring-daw-green/10 focus:border-daw-green/40 disabled:bg-slate-50 disabled:cursor-not-allowed"
                            value={marker.categoryId ? String(marker.categoryId) : ""}
                            onChange={(e) => updateMarker(index, "categoryId", e.target.value)}
                            disabled={!isEditing}
                          >
                    ...
                    <div className="space-y-3 bg-slate-50/50 p-3 rounded-lg border border-slate-100">
                      <div className="space-y-1">
                        ...
                        <input
                          className="w-full text-xs bg-white border border-slate-200 rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-daw-green/10 focus:border-daw-green/40 transition-all disabled:opacity-75 disabled:bg-slate-50 font-medium"
                          value={marker.desc || ""}
                          onChange={(e) => updateMarker(index, "desc", e.target.value)}
                          disabled={!isEditing}
                          placeholder="Contoh: 15 MW"
                        />
                      </div>

                      {/* Magic Translation for Keterangan */}
                      <MagicTranslationField
                        label="Keterangan (Indonesian)"
                        value={marker.terjemahanDesc || ""}
                        onChange={(v) => updateMarker(index, "terjemahanDesc", v)}
                        originalText={marker.desc}
                        disabled={!isEditing}
                      />
                    </div>

                    {/* Row 3: Google Maps Link */}
                    <div className="space-y-1 font-sans">
                      <label className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block">
                        Link Peta / Google Maps URL
                      </label>
                      <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-slate-200 focus-within:ring-2 focus-within:ring-daw-green/10 focus-within:border-daw-green/40 transition-all">
                        <LinkIcon className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                        <input
                          className="w-full bg-transparent outline-none font-mono text-[10px] text-slate-600 disabled:opacity-70"
                          value={marker.mapUrl || ""}
                          onChange={(e) => updateMarker(index, "mapUrl", e.target.value)}
