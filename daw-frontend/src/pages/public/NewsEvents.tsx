          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-6 mb-12 border-b border-slate-100 pb-8">
            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-2">
              <button
                ...
                className={`px-5 py-2.5 rounded-full text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all ${activeCategory === "All" ? "bg-daw-green text-white shadow-md shadow-green-900/20" : "bg-slate-50 text-slate-500 hover:bg-slate-100"}`}>
                All Categories
              </button>
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-5 py-2.5 rounded-full text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all ${activeCategory === cat.id ? "bg-daw-green text-white shadow-md shadow-green-900/20" : "bg-slate-50 text-slate-500 hover:bg-slate-100"}`}>
                  {cat.name}
                </button>
              ))}
            </div>
                    ...
                    {gridArticles.map((article, idx) => (
                      <ScrollReveal
                        ...
                        <Link
                          ...
                          <div
                            className={`relative bg-slate-100 overflow-hidden shrink-0 ${idx % 3 === 0 ? "w-full aspect-[4/3]" : "w-[35%] md:w-full aspect-square md:aspect-[4/3]"}`}>
                            {article.cover_image ? (
                              <img
                          ...
                          <div
                            ...
                            <h3
                              className={`font-bold text-slate-900 group-hover:text-daw-green transition-colors leading-snug line-clamp-2 ${idx % 3 === 0 ? "text-xl mb-4" : "text-sm sm:text-base md:text-xl mb-2 md:mb-4"}`}>
                              {article.title}
                            </h3>
                            <p
                              className={`text-slate-500 leading-relaxed flex-1 ${idx % 3 === 0 ? "text-sm line-clamp-3 mb-5" : "text-xs line-clamp-2 mb-2 md:mb-5 hidden md:block"}`}>
                              {article.excerpt}
                            </p>
                            <div
                              className={`mt-auto items-center text-[10px] md:text-xs font-black uppercase tracking-widest text-daw-green ${idx % 3 === 0 ? "pt-5 border-t border-slate-50 flex" : "pt-2 md:pt-6 md:border-t border-slate-50 hidden md:flex"}`}>
                              Read Article{" "}
