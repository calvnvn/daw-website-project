...
export default function AboutLivePreview({ type, data, extraData }: AboutLivePreviewProps) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div>
              ...
              <p className="font-serif text-xl text-slate-900 leading-relaxed whitespace-pre-line">
                {data?.missionText ? renderHighlightedText(data.missionText) : "Mission text here."}
              </p>
            </div>
            <div>
              <h3 className="text-[10px] font-sans font-bold text-slate-400 uppercase tracking-widest mb-3">
              ...
              <p className="font-serif text-xl text-slate-900 leading-relaxed whitespace-pre-line">
                {data?.visionText ? renderHighlightedText(data.visionText) : "Vision text here."}
              </p>
            </div>
          </div>
        </div>
          ...
          {companyHistory.length > 0 ? (
            <div className="space-y-10">
              {companyHistory.map((item: any, i: number) => (
                <div key={item.id || i} className="relative pl-14 md:pl-20 group">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 lg:p-12 animate-in fade-in duration-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {pillars.map((pillar: any, i: number) => {
             const Icon = ICON_MAP[pillar.iconId?.toLowerCase()] || Icons.CheckCircle;
             return (
               <div key={pillar.id || i} className="p-6 border border-slate-100 bg-slate-50/50 rounded-2xl flex flex-col">
  ...
  if (type === "management") {
    ...
    const executiveDirectors = team.filter((p: any) => p.level === "director").sort((a: any, b: any) => a.order - b.order);
    const divisionHeads = team.filter((p: any) => p.level === "division").sort((a: any, b: any) => a.order - b.order);

    return (
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-8 lg:p-12 animate-in fade-in duration-300">
        {chairman && (
          <div className="mb-10">
            <h2 className="font-serif text-3xl text-slate-900 mb-8">Board of Directors</h2>
            <div className={`grid grid-cols-1 ${chairman.photoUrl || chairman.previewUrl ? "md:grid-cols-12" : ""} gap-8 items-center`}>
              {(chairman.photoUrl || chairman.previewUrl) && (
                <div className="md:col-span-5 lg:col-span-4">
                  <div className="aspect-[3/4] overflow-hidden rounded-xl border border-slate-100 shadow-sm">
                    <img src={chairman.previewUrl || getCleanImageUrl(chairman.photoUrl)} className="w-full h-full object-cover" alt="Chairman" />
                  </div>
                </div>
              )}
              <div className={`${chairman.photoUrl || chairman.previewUrl ? "md:col-span-7 lg:col-span-8" : "col-span-1"}`}>
                <h3 className="font-serif font-bold text-2xl text-slate-900 mb-2">{chairman.name || "Name"}</h3>
                <p className="font-sans text-[11px] font-bold text-daw-green uppercase tracking-[0.2em] mb-4">{chairman.role || "Role"}</p>
                <div className="w-8 h-1 bg-daw-green mb-6 rounded-full"></div>
                <p className="font-sans text-slate-600 text-[13px] leading-relaxed whitespace-pre-line">{chairman.description || "Description"}</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="mb-10 border-t border-slate-100 pt-10">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {executiveDirectors.map((person: any, i: number) => (
                  <div key={person.id || i} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col">
                    <div className="w-16 h-16 mb-6 rounded-full overflow-hidden border-[2px] border-slate-50 shadow-sm bg-daw-green/5 flex items-center justify-center">
                      {(person.photoUrl || person.previewUrl) ? (
                        <img src={person.previewUrl || getCleanImageUrl(person.photoUrl)} className="w-full h-full object-cover" alt="Director" />
                      ) : (
                    ...
                    <h3 className="font-serif font-bold text-xl text-slate-900 mb-1">{person.name || "Name"}</h3>
                    <p className="font-sans text-[10px] font-bold text-daw-green uppercase tracking-[0.2em] mb-4">{person.role || "Role"}</p>
                    <p className="font-sans text-slate-600 text-[13px] leading-relaxed whitespace-pre-line">{person.description}</p>
                  </div>
                ))}
             </div>
          </div>
        ...
        {divisionHeads.length > 0 && (
          <div className="pt-10 border-t border-slate-100">
            <h2 className="font-serif text-2xl text-slate-900 mb-6">Division Heads</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              {divisionHeads.map((person: any, i: number) => (
                <div key={person.id || i} className="bg-slate-50/50 p-6 rounded-2xl border border-transparent">
                   <div className="w-12 h-12 mb-4 rounded-xl overflow-hidden shadow-sm bg-white border border-slate-100 flex items-center justify-center">
                      {(person.photoUrl || person.previewUrl) ? (
                        <img src={person.previewUrl || getCleanImageUrl(person.photoUrl)} className="w-full h-full object-cover" alt="Head" />
                      ) : (
                        <span className="font-serif font-bold text-sm text-slate-400">{getInitials(person.name)}</span>
                      )}
                   </div>
                   <h3 className="font-serif font-bold text-lg text-slate-900 mb-1">{person.name || "Name"}</h3>
                   <p className="font-sans text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-3">{person.role || "Role"}</p>
                   <p className="font-sans text-slate-600 text-[12px] leading-relaxed whitespace-pre-line line-clamp-4">{person.description}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {team.length === 0 && (
           <div className="py-12 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
             <p className="text-slate-400 text-sm">No management team members available.</p>
           </div>
        )}
      </div>
    );
        ...
        <div className="flex flex-col gap-6">
          {achievements.length > 0 ? (
            achievements.map((item: any, i: number) => {
              ...
              return (
                <div key={item.id || i} className="relative bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col md:flex-row gap-5 items-center overflow-hidden">
                  ...
                  <div className="w-full md:w-[35%] shrink-0 z-10">
                    { (item.previewUrl || item.imageUrl) && (
                      <div className="aspect-[16/10] rounded-xl overflow-hidden border border-slate-100">
                        <img src={item.previewUrl || getCleanImageUrl(item.imageUrl)} className="w-full h-full object-cover" alt="Achievement" />
                      </div>
                    )}
                  </div>
                  <div className="w-full md:w-[65%] flex flex-col justify-center py-2 z-10">
                    ...
                    <h3 className="font-serif text-lg md:text-xl text-slate-900 leading-[1.3] mb-2">{item.title || "Achievement Title"}</h3>
                    <p className="font-sans text-slate-600 leading-relaxed text-[12px] whitespace-pre-line">{item.description || "Description goes here."}</p>
                  </div>
                </div>
              )
            })
             ...
             <div className="py-12 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
               ...
               <p className="text-slate-400 text-sm">No achievements available.</p>
             </div>
          )}
        </div>
      </div>
